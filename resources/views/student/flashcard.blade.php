...
<div class="container-fluid flashcard-container border border-dark rounded border-2">
    <div class="row justify-content-center">
        <div class="col-md-12 col-lg-12 mb-4 d-flex justify-content-between border-bottom border-2">
            <h2 class="page-title m-0">{{ $course->name }} Flashcards</h2>
            ...
            <div class="flashcard-card" data-id="{{ $flashcard->id }}">
                <div class="flashcard-inner">
                    <div class="flashcard-front">
                        <h5 class="flashcard-question">{!! $flashcard->question !!}</h5>
                    </div>
                    <div class="flashcard-back">
                        <h5 class="flashcard-answer">{!! $flashcard->answer !!}</h5>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-12 col-lg-12 mb-4">
            <div class="">
                <div class="d-flex justify-content-between">
                    ...
                    <a href="{{ $exams->previousPageUrl() . '&course_id=' . $course->id }}" class="btn btn-primary pagination-btn prev-btn">Previous</a>
                    @else
                    <button class="btn btn-secondary pagination-btn prev-btn" disabled>Previous</button>
                    @endif

                    @if ($exams->nextPageUrl())
                    <a href="{{ $exams->nextPageUrl() . '&course_id=' . $course->id }}" class="btn btn-primary pagination-btn next-btn">Next</a>
                    @else
    ...
    <div class="text-center mt-4">
        <a href="{{ route('flash.card') }}" class="btn btn-secondary">Back to Courses</a>
    </div>
</div>
...
<style>
    ...
    .flashcard-back {
        ...
        font-size: 1.8rem;
        /* Larger font size for questions/answers */
        font-weight: 600;
        line-height: 1.4;
    }
    ...
    .flashcard-answer {
        font-size: 1.6rem;
        /* Slightly smaller than question, but still large */
    }
    ...
    .pagination-controls {
        ...
        margin-top: 20px;
        /* Reduced margin-top as wrapper will have padding */
        padding: 0;
    }
    ...
    .pagination-wrapper {
        background-color: var(--white);
        padding: 20px 30px;
        /* Added padding to the wrapper */
        border-radius: 15px;
        /* Rounded corners for the wrapper */
        box-shadow: 0 10px 25px rgba(0, 0, 0, 0.08);
        /* Subtle shadow for the wrapper */
        margin-top: 30px;
        /* Space above the wrapper */
        margin-bottom: 20px;
